...
if( class_exists('acf') ) {
    $hero_bg = get_field('video_background');
    $hero_t = get_field('hero_style');
    $hero_title = get_field('hero_title');
    $hero_align = get_field('text_align');
    $hero_desc = get_field('hero_description');
    $u_video = get_field('v_upload');
    $e_video = get_field('embed_video_url');
...
<?php if( class_exists('acf') && !empty($hero_bg) ) : ?>
    <?php if ($hero_bg == 'upload'): ?>
        <section class="hero hero--overlay-layer background-image jarallax" data-speed="<?php echo esc_attr ($p_s); ?>" data-jarallax-video="mp4:<?php echo esc_url( $u_video['url'] );?>">
        <?php else: ?>
        <section class="hero hero--overlay-layer background-image jarallax" data-jarallax-video="<?php echo esc_url( $e_video );?>">
    <?php endif ?>
<?php else: ?>
